...
test.describe('Funcionalidade X', () => { test('deve realizar ação esperada', async ({ page }) => { // Arrange await page.goto('/rota');
// Act await page.getByRole('button', { name: 'Enviar' }).click();
// Assert await expect(page.getByRole('heading', { name: 'Sucesso' })).toBeVisible(); });});
Tipagem
